...
router.post("/csv", authenticate, upload.any(), async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    for (const row of records) {
      try {
        ...
        const existing = await pool.query(
          "SELECT id FROM students WHERE email = $1 AND instructor_id = $2",
          [email, instructorId]
        );
        ...
        if (existing.rows.length > 0) {
          ...
          const result = await pool.query(
            `INSERT INTO students (name, email, cohort, gender, disability, instructor_id)
             VALUES ($1, $2, $3, $4, $5, $6) RETURNING id`,
            [name, email, cohort, gender, "N", instructorId]
          );
        ...
        if (engExists.rows.length === 0) {
          await pool.query(
            `INSERT INTO engagement 
             ...
             VALUES ($1, $2, $3, $4, $5, $6)`,
            [studentId, week, loginCount, forumPosts, videoMinutes, submissions]
          );
          engagementLogged++;
        }
    ...
    res.status(500).json({ error: "Failed to process CSV", details: String(err) });
  }
});
