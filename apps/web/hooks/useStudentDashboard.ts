...
export function useStudentDashboard(token: string | null) {
  ...
  const getToken = () => localStorage.getItem("student_token") || token || "";
  ...
  const logForumPost = () => {
    fetch(`${API}/api/student-auth/activity/forum`, {
      ...
      setData((prev) => prev ? {
        ...prev,
        engagement: { ...prev.engagement!, forum_posts: (prev.engagement?.forum_posts ?? 0) + 1 }
      } : prev);
    }).catch(console.error);
  };
  ...
  const logVideo = (minutes: number) => {
    fetch(`${API}/api/student-auth/activity/video`, {
      ...
      setData((prev) => prev ? {
        ...prev,
        engagement: { ...prev.engagement!, video_watch_minutes: (prev.engagement?.video_watch_minutes ?? 0) + minutes }
      } : prev);
    }).catch(console.error);
  };
  ...
  const submitAssignment = (score: number) => {
    fetch(`${API}/api/student-auth/activity/assignment`, {
      ...
      setData((prev) => prev ? {
        ...prev,
        assessments: [{ id: crypto.randomUUID(), score, submitted: true, submitted_at: new Date().toISOString(), week: 0 }, ...prev.assessments]
      } : prev);
    }).catch(console.error);
  };
