...
interface DashboardData {
  student: { id: string; name: string; email: string; cohort: string; enrolled_at: string };
  riskScore: { risk_score: number; risk_label: string; top_factors: Array<{ feature: string; impact: number }>; suggestion: string } | null;
  engagement: { login_count: number; forum_posts: number; video_watch_minutes: number; assignment_submissions: number } | null;
  assessments: Array<{ id: string; score: number; submitted: boolean; created_at: string }>;
}
...
export default function StudentDashboardPage() {
  ...
  useEffect(() => {
    ...
    fetchDashboard(token || storedToken || "");
  }, []);
  ...
  if (!data) {
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex items-center justify-center">
        <p className="text-red-400 text-sm">Failed to load dashboard. Please try again.</p>
      </div>
    );
  }

  const avgScore = data.assessments.length
    ? Math.round(data.assessments.reduce((a, b) => a + b.score, 0) / data.assessments.length)
    : null;
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      ...
      <nav className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 px-6 py-4">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="font-medium text-gray-900 dark:text-white">Student AI</span>
          </div>
          <div className="flex items-center gap-4">
            <span className="text-sm text-gray-500 dark:text-gray-400">{data.student.name}</span>
            <button
      ...
      <main className="max-w-5xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-2 gap-6">
          {/* Risk Score */}
          {data.riskScore ? (
            <div className={`rounded-2xl border p-6 ${riskBg(data.riskScore.risk_label)}`}>
              <p className="text-xs text-gray-500 uppercase tracking-wide mb-3">Your Risk Score</p>
              <div className="flex items-center gap-3 mb-4">
                <span className={`text-5xl font-bold ${riskColor(data.riskScore.risk_label)}`}>
                  {Math.round(data.riskScore.risk_score * 100)}%
                </span>
                <span className={`text-sm px-3 py-1 rounded-full font-medium capitalize ${riskColor(data.riskScore.risk_label)} bg-white/50 dark:bg-black/20`}>
                ...
                >
                  ...
                  <Bar dataKey="value" radius={4}>
                    {data.riskScore.top_factors.map((_, i) => (
                      <Cell key={i} fill={riskBarColor(data.riskScore!.risk_label)} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
